...
class SpaEncoder(nn.Module):
    ...
    def forward(self, x, sub_ids=None):
        ...
        emb_out = self.enc_embedding(x_enc, None,
                                     subject_ids=sub_ids)  # emb_out : [batch_size , (channels + 1) , d_model]
        # encode EncoderLayer中做了类似Residual的处理
        enc_out, attn = self.encoder(emb_out, attn_mask=None)  # enc_out : [batch_size , (channels + 1) , d_model]
        return enc_out, attn
...
class SubmodalProj(nn.Module):
    def __init__(self, configs, channels = 64, extra_submodal_dim = 32, embedding_dim = 512 , proj_clip_dim = 768):
        super().__init__()
