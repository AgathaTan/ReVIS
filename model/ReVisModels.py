...
class SpaEncoder(nn.Module):
    ...
    def __init__(self, configs):
        super().__init__()
        self.norm = nn.LayerNorm(configs.seq_len)
        # self.freq_seq_len = configs.freq_seq_len
        #self.enc_embedding = DataEmbedding(configs.seq_len, configs.d_model, dropout=configs.dropout,
    ...
    def forward(self, x, sub_ids=None):
        ...
        x_enc = self.norm(x_enc)
        # embedding
        # emb_out = self.enc_embedding(x_enc, None, subject_ids=sub_ids)  # emb_out : [batch_size , channels , d_model]
        emb_out = self.enc_embedding(x_enc)  # emb_out : [batch_size , channels , d_model]
        # encode EncoderLayer中做了类似Residual的处理
        enc_out, attn = self.encoder(emb_out, attn_mask=None)  # enc_out : [batch_size , channels , d_model]
        return enc_out, attn
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, x, sub_ids = None):
        ...
        emb_out = self.embedding(z)                                                                                          # emb_out: [batch_size , channels , patch_num , patch_d_model]
        emb_out = torch.reshape(emb_out, (emb_out.shape[0] * emb_out.shape[1], emb_out.shape[2], emb_out.shape[3]))   # emb_out: [batch_size * channels , patch_num , patch_d_model]
        emb_out = self.dropout(emb_out + self.W_pos)                                                                         # emb_out: [batch_size * channels , patch_num , patch_d_model]
        #encode
        enc_out, attn = self.PatchEncoder(emb_out)                                                                           # enc_out: [batch_size * channels , patch_num , patch_d_model]
        enc_out = torch.reshape(enc_out, (batch_size, -1 ,enc_out.shape[-2],enc_out.shape[-1]))                       # enc_out: [batch_size , channels , patch_num , patch_d_model]
        out = self.flatten(enc_out)                                                                                         # enc_out: [batch_size , channels , patch_num * patch_d_model]
...
class SemanticProj(nn.Module):
    def __init__(self, configs, channels = 63, clip_text_dim = 77 , embedding_dim = 512 , proj_clip_dim = 768):
        super().__init__()
...
class SubmodalProj(nn.Module):
    def __init__(self, configs, channels = 63, extra_submodal_dim = 16, embedding_dim = 512 , proj_clip_dim = 768):
        super().__init__()
        self.proj = torch.nn.Sequential(
            torch.nn.Linear(embedding_dim, proj_clip_dim),
            nn.Conv1d(in_channels=channels, out_channels=extra_submodal_dim, kernel_size=1),
